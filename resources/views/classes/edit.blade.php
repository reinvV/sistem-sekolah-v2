...
<main class="mx-auto w-full max-w-2xl flex-1 px-6 py-10">
    <div class="mb-8 border-b border-[#E5E3DB] pb-5">
        ...
        <p class="mt-1 text-sm text-slate-500">
            Memperbarui data untuk <span class="font-medium text-[#16213A]">{{ $class['name'] }}</span>.
        </p>
    </div>
    ...
    <form action="{{ route('classes.update', $class['id']) }}" method="POST" class="space-y-6 border border-[#E5E3DB] bg-white p-8">
        ...
        <div>
            ...
            <select id="major" name="major"
                class="w-full border border-[#D9D6CD] bg-[#FCFBF8] px-3.5 py-2.5 text-sm focus:border-[#A16207] focus:bg-white focus:outline-none">
                <option value="AKL" {{ old('major', $class['major']) == 'AKL' ? 'selected' : '' }}>AKL</option>
                <option value="TKJ" {{ old('major', $class['major']) == 'TKJ' ? 'selected' : '' }}>TKJ</option>
                <option value="BiD" {{ old('major', $class['major']) == 'BiD' ? 'selected' : '' }}>BiD</option>
            </select>
            @error('major')
                <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
            @enderror
        ...
        <div>
            ...
            <input type="text" id="homeroom_teacher" name="homeroom_teacher" value="{{ old('homeroom_teacher', $class['homeroom_teacher']) }}"
                class="w-full border border-[#D9D6CD] bg-[#FCFBF8] px-3.5 py-2.5 text-sm focus:border-[#A16207] focus:bg-white focus:outline-none">
