...
<main class="mx-auto w-full max-w-2xl flex-1 px-6 py-10">
    <div class="mb-8 border-b border-[#E5E3DB] pb-5">
        ...
        <p class="mt-1 text-sm text-slate-500">
            Memperbarui catatan atas nama <span class="font-medium text-[#16213A]">{{ $teacher['name'] }}</span>.
        </p>
    </div>
    ...
    <form action="{{ route('teachers.update', $teacher['id']) }}" method="POST" class="space-y-6 border border-[#E5E3DB] bg-white p-8">
        ...
        <div>
            ...
            <select id="status" name="status"
                ...
                <option value="Non-Aktif" {{ old('status', $teacher['status']) == 'Non-Aktif' ? 'selected' : '' }}>Non-Aktif</option>
            </select>
            @error('status')
                <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
            @enderror
