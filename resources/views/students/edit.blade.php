...
<main class="mx-auto w-full max-w-2xl flex-1 px-6 py-10">
    ...
    <div class="mb-8 border-b border-[#E5E3DB] pb-5">
        ...
        <p class="mt-1 text-sm text-slate-500">
            Memperbarui catatan atas nama <span class="font-medium text-[#16213A]">{{ $student['name'] }}</span>.
        </p>
    </div>

    <form action="{{ route('students.update', $student['id']) }}" method="POST" class="space-y-6 border border-[#E5E3DB] bg-white p-8">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <select id="major" name="major"
                ...
                <option value="BiD" {{ old('major', $student['major']) == 'BiD' ? 'selected' : '' }}>BiD</option>
            </select>
        </div>

        <div>
            <label for="class" class="mb-1.5 block text-xs font-semibold uppercase tracking-[0.1em] text-[#16213A]">Kelas</label>
